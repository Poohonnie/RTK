...
struct SATOBS
{
    ...
    double CN0Ratio{};
    double locktime{};
    unsigned short halfCycle{};
 
	bool valid{};  // 通道状态
 
	bool check();  // 检测双频伪距和相位数据是否完整有效
};
...
public:
	...
	static unsigned short U2(unsigned char* buf);  // 读取两个字节, 返回无符号整数
	static unsigned int U4(unsigned char* buf);  // 读取四个字节, 返回无符号整数
	static float R4(unsigned char* buf);  // 读取四个字节, 返回浮点数
	static double R8(unsigned char* buf);  // 读取八个字节, 返回双精度数
